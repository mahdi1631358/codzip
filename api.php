...
switch (true) {
    ...
    case (preg_match('/api\/get-database$/', $path) && $method === 'GET'):
        try {
            // Load base JSON from system state
            $stmt = $pdo->prepare("SELECT `state_value` FROM `system_state` WHERE `state_key` = 'database'");
            $stmt->execute();
            $row = $stmt->fetch();
            $baseJSON = $row ? $row['state_value'] : $defaultSeed;
            
            $db = json_decode($baseJSON, true);
            if (!is_array($db)) {
                $db = json_decode($defaultSeed, true);
            }

            // 1. Fetch dynamic, live users from the SQL table
            $stmtUsers = $pdo->prepare("SELECT id, phone, full_name, role, is_super_admin, city, created_at FROM users ORDER BY id DESC");
            $stmtUsers->execute();
            ...
            // 4. Harmonize and auto-merge SQL technicians (role='technician') into the state technicians array
            $existingTechs = isset($db['technicians']) && is_array($db['technicians']) ? $db['technicians'] : [];
            $existingTechPhones = array_column($existingTechs, 'phone');
            
            foreach ($sqlUsers as $u) {
                if ($u['role'] === 'technician') {
                    if (!in_array($u['phone'], $existingTechPhones)) {
                        $existingTechs[] = [
                            "id" => "tech_" . $u['id'],
                            "name" => $u['full_name'] ? $u['full_name'] : 'تکنسین گرامی',
                            "phone" => $u['phone'],
                            "password" => "", // Secure password remains in salt-hashed sql users table
                            "specialty" => ["سایر لوازم"],
                            "rating" => 5.0,
                            "completedOrders" => 0,
                            "balance" => 0,
                            "isVerified" => false, 
                            "activeLocation" => $u['city'] ? $u['city'] : "نامشخص",
                            "documents" => [],
            ...
            // Output the synchronized state package
            header('Content-Type: application/json; charset=utf-8');
        ...
        try {
            ...
            foreach ($mergeableKeys as $key) {
                if (isset($requestData[$key]) && is_array($requestData[$key])) {
                    $currentList = isset($currentDb[$key]) && is_array($currentDb[$key]) ? $currentDb[$key] : [];
                    $reqList = $requestData[$key];
                    
                    if (empty($reqList)) {
                        // Check if it's a global reset operation (usually contains multiple empty arrays like reset database button)
                        $emptyCount = 0;
                        ...
                        $currentDb[$key] = array_values($mergedList);
                    }
                    // Prevent standard array_merge from overwriting our safe merged list
